...
class User extends UserBase
{
    ...
    public $belongsToMany = [
        'groups' => ['HS\Models\User\UserGroup', 'table' => 'users_groups']
    ];
    ...
    public function attemptActivation($code)
    {
        ...
        Event::fire('app.user.activate', [$this]);
    ...
    public function afterLogin()
    {
        ...
        if ($this->trashed()) {
            ...
            Mail::sendTo($this, 'app::mail.user.reactivate', [
                'name' => $this->name
            ]);

            Event::fire('app.user.reactivate', [$this]);
        }
        else {
            parent::afterLogin();
        }

        Event::fire('app.user.login', [$this]);
    }
    ...
    public function afterDelete()
    {
        if ($this->isSoftDelete()) {
            Event::fire('app.user.deactivate', [$this]);
            return;
        }
    ...
    protected function getNotificationVars()
    {
         ...
         * Extensibility
         */
        $result = Event::fire('app.user.getNotificationVars', [$this]);
    ...
    /**
     * Sends an invitation to the user using template "app::mail.invite".
     * @return void
     */
    protected function sendInvitation()
    {
        Mail::sendTo($this, 'app::mail.invite', $this->getNotificationVars());
    }
